 ...
 * against. Biome sources carry no registry reference, so the owner is read off
 * one of the holders they were handed.
 */
public final class MythicOverlayLookup {
     ...
     * into the generated files. Reflective because the class is loader-only.
     */
     ...
     *
     * <p>Datapack-loaded holders are not owned by the registry itself but by the
     * lookup view it hands to {@code RegistryOps}. {@code Registry} implements
     * {@link HolderGetter} through {@code HolderLookup.RegistryLookup}, so the
     * single {@link HolderGetter} case covers both.
     */
    ...
    public static Holder<Biome> resolve(Holder<Biome> sample, ResourceKey<Biome> key) {
        ...
        if (owner instanceof HolderGetter<?> getter) {
            return ((HolderGetter<Biome>) getter).get(key).map(h -> (Holder<Biome>) h).orElse(null);
        }
        return null;
    }
}
